...
import { createNewsletterForCuration } from './actions'
...
export default function CreateNewsletterModal() {
  ...
  const [state, formAction, isPending] = useActionState(
    async (prevState: any, formData: FormData) => {
      try {
        await createNewsletterForCuration(formData)
        setIsOpen(false)
        return { success: true }
      } catch (error) {
        return { success: false, error: error instanceof Error ? error.message : 'Failed to create newsletter' }
      }
    },
    { success: false }
  )
          ...
          <div className="w-full max-w-md rounded-xl border border-(--color-card-border) bg-(--color-card-bg) p-5">
            <div className="mb-4">
              ...
              <p className="type-caption text-(--color-text-secondary)">
                Add a new newsletter draft for curation.
              </p>
            </div>
            ...
            <form action={formAction} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="rounded-md bg-green-600 px-4 py-2 type-caption font-medium text-white hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-green-600 dark:hover:bg-green-500"
                >
